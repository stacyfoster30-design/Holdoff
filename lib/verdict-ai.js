...
function analyzeLocally(message) {
  ...
  const hasUrgency    = /now|tonight|today|please|need to|have to|asap|right now|still|yet/.test(m);
  ...
  // Determine verdict
  if (urgencyScore >= 4 || hasAngry || hasPanic) {
    return {
      verdict: 'HOLD',
