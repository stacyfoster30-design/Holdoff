...
if (!JWT_SECRET) {
  throw new Error('[auth] Missing JWT_SECRET. Refusing to start with an ephemeral or hardcoded signing secret. Set a cryptographically random 32+ byte JWT_SECRET before enabling auth.');
}
if (JWT_SECRET.length < 32) {
  throw new Error('[auth] JWT_SECRET is too short. Use a cryptographically random value of at least 32 characters.');
}
 ...
 * NOTE: refreshPayload is resolved synchronously from the access cookie only.
 * For DB-backed refresh token validation, use getRefreshTokenFromCookie() directly (async).
 */
function getCookieTokens(req) {
  return {
    accessPayload: getAccessTokenFromCookie(req),
    // getRefreshTokenFromCookie is async (DB lookup) — expose null here so callers
    // that only need the access JWT stay synchronous. Use getRefreshTokenFromCookie()
    // directly wherever the refresh DB path is needed.
    refreshPayload: null,
  };
}
