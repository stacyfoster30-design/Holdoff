...
const JWT_SECRET = process.env.JWT_SECRET || process.env.SESSION_SECRET || crypto.randomBytes(48).toString('hex');
if (!process.env.JWT_SECRET && !process.env.SESSION_SECRET) {
  console.warn('[auth] JWT_SECRET/SESSION_SECRET is not set — using an ephemeral boot secret. Set JWT_SECRET in Render before enabling login/payment sessions.');
}
